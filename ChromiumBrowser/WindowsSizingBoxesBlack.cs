...
{
    public class WindowsSizingBoxesBlack
    {
        ...
        protected Image _githubImage = null;
        protected Brush _minimizeMaximizeButtonHighlight = new SolidBrush(Color.FromArgb(27, Color.White));
        ...
        public Rectangle _githubButtonArea = new Rectangle(-45, 0, 45, 29);
        public WindowsSizingBoxesBlack(TitleBarTabs parentWindow)
        {
            _parentWindow = parentWindow;
            _githubImage = LoadSvg(Encoding.UTF8.GetString(Resources.github_mark_white), 14, 14);
        ...
        public int Width
        {
            get
            {
                return _minimizeButtonArea.Width + _maximizeRestoreButtonArea.Width + _closeButtonArea.Width + _githubButtonArea.Width;
            }
        }

        public bool Contains(Point cursor)
        {
            return _minimizeButtonArea.Contains(cursor) || _maximizeRestoreButtonArea.Contains(cursor) || _closeButtonArea.Contains(cursor) || _githubButtonArea.Contains(cursor);
        }
        ...
        public void Render(Graphics graphicsContext, Point cursor)
        {
            ...
            _githubButtonArea.X = right - 135-45;
            _minimizeButtonArea.X = right - 135;
            _maximizeRestoreButtonArea.X = right - 90;
            _closeButtonArea.X = right - 45;

            if (_githubButtonArea.Contains(cursor))
            {
                graphicsContext.FillRectangle(_minimizeMaximizeButtonHighlight, _githubButtonArea);
            }
            else if (_minimizeButtonArea.Contains(cursor))
            {
            ...
            graphicsContext.DrawImage(_githubImage, _githubButtonArea.X + 17 -2, _githubButtonArea.Y + 9 -2);
        }
